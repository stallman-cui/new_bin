...
exec_command='t=$(expr $(date +%s) / 300 - 1);t=$(date --date @$(echo $t*300|bc) +%Y%m%d%H%M); for g in /home/mhgame/games/*; do find $g/log/gamelog -name mh_$t.log | xargs sed -e "s/^/$(basename $g)\t/"; done 2>/dev/null'
